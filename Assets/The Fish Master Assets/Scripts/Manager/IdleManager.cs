...
public class IdleManager : MonoBehaviour
{
    ...
    // 追加：オフライン収益の逓減設定
    [SerializeField] private float offlineBasePerMin = 3f;   // 初期の基礎値
    [SerializeField] private float offlineK = 0.35f;         // 逓減の強さ（0.25〜0.5推奨）
    [SerializeField] private int maxOfflineTotal = 5000;     // （任意）安全上限も併用推奨
    ...
    public float OfflinePerMinuteFloatForUI
    {
        ...
        {
            // 逓減式と同じロジックで、小数のまま返す
            float basePer = offlineBasePerMin;   // 例: 3
            float per = basePer + offlineK * Mathf.Sqrt(offlineLevel); // 例: 0.35 * sqrt(Lv)
    ...
    private void RecalcCosts()
    {
        ...
        if (BuyLengthsButton != null)
        {
            ...
            SetButtonLabel(BuyLengthsButton, canLevelUp ? $"購入 ({lengthCost})" : "MAX");
        }
        ...
        if (BuyStrengthButton != null)
        {
            ...
            SetButtonLabel(BuyStrengthButton, canLevelUp ? $"購入 ({strengthCost})" : "MAX");
        }
        ...
        if (BuyOfflineEarningsButton != null)
        {
            ...
            SetButtonLabel(BuyOfflineEarningsButton, canLevelUp ? $"購入 ({offlineEarningsCost})" : "MAX");
        }
    }
    ...
    private void ComputeOfflineAndShow()
    {
                ...
                DateTimeStyles.RoundtripKind, out var lastUtc))
        {
            ...
            totalGain = Mathf.Min(totalGain, maxOfflineTotal);  // 保険
    ...
    public bool BuyLength()
    {
        ...
        if (lengthLevel >= MaxLevel)  // ★ 上限チェック
            return false;
    ...
    {
        if (strengthLevel >= MaxLevel)  // ★ 上限チェック
            return false;
    ...
    {
        if (offlineLevel >= MaxLevel)  // ★ 上限チェック
            return false;
